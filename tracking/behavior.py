        ...
        for i, (lab, length) in enumerate(runs):
            if length < min_run:
                if i == 0:
                    ...
                    # Merge into whichever neighbor is LONGER (more
                    # stable), not always the previous one -- a brief
                    # blip sitting between a long, confident run and a
                    # short, uncertain one is more likely noise within
                    # the STABLE run's bout than a genuine transition.
