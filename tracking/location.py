...
def detect_mouse(
    ...
    # being close to wherever tracking last was. Only ever filters when
    # there's more than one candidate AND at least one survives filtering,
    # so it never turns a genuine single real detection into "nothing
    # found" (protects real freezing/immobility the same way the
    # two_mouse.py version does).
    if motion_mask is not None and len(candidates) > 1:
    ...
    while frame_number <= end_frame:
        ...
        if build_display:
            ...
            if status == "Tracked":
                ...
                if entry_list:
                    status_line += "  | Entry: " + ", ".join(entry_list)
